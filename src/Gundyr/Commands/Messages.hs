...
messageGroup :: (BotC r, Member DBEff r) => Sem (DSLState r) ()
messageGroup = void
...
sendMessage :: (BotC r, Member DBEff r) => Sem (DSLState r) ()
sendMessage = void
  . help (const "send a new message to the channel, and store the message in db")
  . command @'[ Named "channel" (Snowflake Channel)
              , Named "label" Text
              , Named "message" (KleenePlusConcat Text)
              ] "send" $ \ctx ch label msgText -> do
...
listBotMsg :: (BotC r, Member DBEff r) => Sem (DSLState r) ()
listBotMsg = void
...
showBotMsg = void
  ...
  . command @'[Named "label" Text] "show" $ \ctx label -> do
...
deleteBotMsg :: (BotC r, Member DBEff r) => Sem (DSLState r) ()
deleteBotMsg = void
  . help (const "delete a bot message")
  . command @'[Named "label" Text] "delete" $ \ctx label -> do
...
editBotMsg :: (BotC r, Member DBEff r) => Sem (DSLState r) ()
editBotMsg = void
  . help (const "edit a bot message")
  . command @'[ Named "label" Text
              , Named "body" Text
              ] "edit" $ \ctx label body -> do
