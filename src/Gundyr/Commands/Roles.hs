...
roleGroup :: (BotC r, Member DBEff r) => Sem (DSLState r) ()
...
roleInfo = void
  ...
  . command @'[ Named "role" (Snowflake Role)
                ...
                res1 <- map (L.unwords . map (mention .(^. #role_id2))) <$>
                  mapM selectUsing [allPrereqs, allContradicts, allRemoves]
                res2 <- map (L.unwords . map (mention .(^. #role_id1))) <$>
                  mapM selectUsing [allPrereqs', allContradicts', allRemoves']
                case (res1, res2) of
